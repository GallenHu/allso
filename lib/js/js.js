...
function google() {
    var url = 'https://www.google.com/search?q=' + $('#soinput').val();
    window.open(url);
}
function doge() {
    var url = 'https://www.dogedoge.com/results?q=' + $('#soinput').val();
    window.open(url);
}
...
if (localStorage["allso_0"] == undefined) {
    set(0, 0);
    obj_list1_buttons.eq(0).removeClass("btn-info").addClass("btn-danger");
}
else {
...
function set(a0b1, set_so) {
    ...
    if (set_so == 0) {
        ...
        set_url[a0b1] = "https://cn.bing.com/search?FORM=BESBTB&q=";
        set_top[a0b1] = -82; set_left[a0b1] = -80; set_foot[a0b1] = 60;
    }
    else if (set_so == 3) {
        set_url[a0b1] = "https://www.so.com/s?ie=utf-8&q=";
...
function change_state(is_zuo) {
    if (localStorage["allso_state"] == 0 ||  localStorage["allso_state"] === undefined)
